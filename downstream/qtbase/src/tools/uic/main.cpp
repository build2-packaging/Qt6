...
{
    // @@ With QT_BOOTSTRAPPED defined (as it is during the upstream UIC
    //    build), QT_DEPRECATED_SINCE(major, minor) is defined to 0 in
    //    qglobal.h (the trail starts with QT_NO_DEPRECATED in
    //    qconfig-bootstrapped.h). As a result the deprecated
    //    qSetGlobalQHashSeed(int) is not emitted because it is wrapped in
    //    `#if QT_DEPRECATED_SINCE(6, 6)`. Therefore use its replacement,
    //    QHashSeed.
    //
