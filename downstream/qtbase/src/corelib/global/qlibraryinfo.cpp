...
static QString getRelocatablePrefix(QLibraryInfoPrivate::UsageMode usageMode)
{
    ...
    // @@ A usageMode of UsedFromQtBinDir is currently only ever passed from
    //    qmake's source code so we won't ever need this code (which causes
    //    compilation failures due to our modifications in qconfig.cpp).
    //
